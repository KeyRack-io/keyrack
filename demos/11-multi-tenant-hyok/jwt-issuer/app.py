"""Minimal JWT issuer for KeyRack multi-tenant HYOK demo.

Generates an RSA-2048 key pair at startup and exposes:
  GET  /.well-known/jwks.json  — public key as JWKS
  POST /token                  — issue a signed JWT with a scope claim
"""
...
def issue_token():
    ...
    # The scope claim is namespaced under the configured claims_namespace
    # (keyrack:) so KeyRack extracts principal attribute "scope".
    if scope:
        claims["keyrack:scope"] = scope
