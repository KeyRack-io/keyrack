...
fn worker_kani_harnesses_exist(root: &Path) {
    ...
    let registry: serde_json::Value =
        serde_json::from_str(&read(&worker.join("verification/kani-harnesses.json")))
    ...
    let registered: std::collections::BTreeSet<_> = registry["harnesses"]
        .as_array()
        .expect("worker harness list")
        .iter()
        .map(|name| name.as_str().expect("harness name"))
        .collect();
    assert!(
        !registered.is_empty(),
        "worker harness registry must not be empty"
    );
    let actual: std::collections::BTreeSet<_> = proofs
        .split("#[kani::proof]")
        .skip(1)
        .map(|part| {
            part.split("fn ")
    ...
    assert_eq!(
        registered, actual,
        "worker proof annotations and registry differ"
    );
    ...
    assert_eq!(
        ...
        "worker proof documentation and registry differ"
    );
}
